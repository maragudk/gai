...
## Step 6: Replace the marker interface with a serializable envelope

**Author:** envelope-builder

### Prompt Context

**Verbatim prompt:** "PR #336 round-trips provider thought signatures through a new `gai.Part.Metadata` field so multi-turn tool use works on Gemini 3.x and Anthropic extended thinking (fixes #250, #256). It is functionally complete and live-verified, but Markus's review found the metadata *shape* wrong: `Metadata` is a marker-interface type (`gai.PartMetadata`) implemented by `google.PartMetadata` and `anthropic.PartMetadata`. That marshals but cannot unmarshal — encoding/json can't restore an interface field — so persisted message history loses the metadata, which defeats the round-trip. Decision: replace it with a concrete, serializable envelope. [...] Replace the `PartMetadata` interface with a concrete struct: a provider discriminator string plus opaque bytes [...] Check how `Part` serializes today (there is a `MarshalText` nearby) and make sure metadata survives every serialization path `Part` supports; if `Part` has custom marshalling that would drop it, fix that too."
**Interpretation:** Keep every behaviour Steps 2-5 pinned, but change the metadata type from an interface to a `{source, bytes}` struct, and make `gai.Part` actually serializable so a persisted history keeps its metadata — including fixing whatever in `Part`'s existing marshalling would drop it.
**Inferred intent:** The feature exists so callers can store a conversation and replay it; a metadata field that cannot be stored is not a fix. Make the round-trip guarantee testable rather than promised.

### What I did

Core (`/chat_complete.go`): `gai.PartMetadata` is now a struct of `Source string` plus `Data []byte`, both JSON-tagged, zero value meaning "no metadata". Its GoDoc states the contract — the producing implementation sets `Source` to its own identifier (conventionally its import path) and is the only one that may interpret `Data`; everyone else treats the envelope as a black box and ignores a `Source` they do not own — and names no client package, per the second review comment on the PR.

The serialization path turned out to be the real work. `Part` implements `encoding.TextMarshaler`, and `encoding/json` prefers that over struct marshalling, so `json.Marshal(part)` produced a display string like `"[thought: ...]"` — every field dropped, not just metadata — and unmarshalling failed outright. So `Part` gained `MarshalJSON`/`UnmarshalJSON` over an unexported `partJSON` wire form carrying `type`, `text`, `data`, `mimeType`, `toolCall`, `toolResult`, and `metadata` (`omitzero`, so an unannotated part stays `{"type":"text","text":"hi"}`). `MarshalText` stays as the short summary form, now documented as such. A `ToolResult.Err` is an interface and cannot be unmarshalled, so it travels as its message and comes back as `errors.New` of the same text — documented on `MarshalJSON`. `UnmarshalJSON` rejects a part missing the content its type requires, so a truncated or hand-edited history errors at the boundary instead of panicking later inside `Part.Text()`.

Clients: `google.PartMetadata`, `anthropic.PartMetadata`, and both `asPartMetadata` helpers are gone. Each package now has a package-private `partMetadataSource` constant holding its own import path. Google puts the `thought_signature` bytes in `Data` verbatim (`newPartMetadata`) and reads them back with a three-line `thoughtSignature(gai.PartMetadata) []byte` returning nil for a foreign or absent source — no `ok` flag needed, since an absent signature and an empty one mean the same thing to the request builder. Anthropic has two fields to carry, so it JSON-encodes a package-private `partMetadata{signature, redactedThinkingData}` into `Data` via `partMetadata.encode()`, and `decodePartMetadata` returns `(partMetadata, bool, error)`: foreign or absent metadata is `(zero, false, nil)` and ignored exactly as before, while bytes tagged as ours that fail to decode return the new package-private `errPartMetadataInvalid`, wrapped as `anthropic: part metadata is not decodable: ...`, span-recorded and span-ended like `errLastMessageEmpty`. Google needs no such error: there is nothing to decode. Both clients also gained a caller-facing paragraph on `ChatComplete` describing how signatures ride on parts, replacing what the deleted exported types used to document.

One consequence needed handling outside the brief. `eval.Sample` holds `[]gai.Part` and is JSON-marshalled into `evals.jsonl`; `Part.MarshalText` was added for exactly that in #168. Giving `Part` a `MarshalJSON` would have turned every eval line into full part objects — and `internal/examples/evals` puts a 327KB JPEG in a data part, so each line would have carried its base64. `eval.Sample` therefore got its own `MarshalJSON` rendering parts through `MarshalText`, keeping `evals.jsonl` byte-identical to before, pinned by a new `TestSample_MarshalJSON`.

Tests: all four external-review findings from Steps 2-5 stay pinned. The two live multi-turn flows (Gemini 3.5 Flash Lite tool use, Sonnet 5 unprompted-thinking resend) were extracted into helpers taking a `persist func(*testing.T, []gai.Message) []gai.Message`, so each now runs as two subtests — once passing parts straight back, once through `marshalAndUnmarshalMessages`. New offline coverage: core round-trip of a part, of a whole message history with every part type, of a tool-result error, and the rejection table; per client, a signed history marshalled to JSON, read back, replayed through the `httptest` transport, asserting the signature reaches the wire (`thoughtSignature` for google, `signature`/`data` for anthropic). The anthropic package had no stub transport before, so Step 6 adds `newStubChatCompleter` (SDK client with `option.WithBaseURL`) and `writeMessageStream`. The moot pointer-form subtests are gone; google gained a positive offline "ignores metadata from another implementation" asserting the foreign envelope never appears in the request body, and anthropic gained "errors on its own metadata that cannot be decoded". The `foreignPartMetadata` types were replaced by a shared `foreignMetadata` var per test package, and each test package spells out its client's source string and wire shape as literals rather than importing them, so a change to the persisted format breaks the tests.

Also updated the metadata line in `/AGENTS.md`, and dropped a stale sentence in google's `newChatCompleter` doc claiming `gai.Part` "does not yet preserve" signatures.

Self-review then ran as two competing agents over the diff — one on public-API correctness, one adversarially trying to break the round-trip from a throwaway module outside the repo. Between them they found six real defects, all now fixed and pinned by tests. Both flagged the same two first: `UnmarshalJSON` had no `default` case, so `{"type":"video"}`, `{"type":""}` and a JSON `null` element all passed the new validation and then hit `panic("unknown part type ...")` inside both clients — precisely what that validation exists to prevent — and `UnmarshalJSON([]byte("null"))` zeroed the receiver instead of being the no-op Go's convention expects. Then: a `ToolResult` error with an empty message vanished through `Err string` + `omitempty`, turning a failed tool result back into a successful one carrying its old content (the reviewer captured the divergence in both clients' request bodies: `"is_error":true,"content":[{"text":""}]` became `"is_error":false,"content":[{"text":"partial output"}]`), now fixed with `Err *string`; `MarshalJSON` happily wrote parts `UnmarshalJSON` would refuse, so a history could be saved and never reloaded; anthropic's `decodePartMetadata` treated its own source with empty bytes as corruption rather than absence, contradicting both its own doc and `gai.PartMetadata`'s; and `Part.MarshalJSON` failed the whole history on truncated tool-call arguments with an opaque `json: error calling MarshalJSON` message. The first four and the sixth are now one shared `partJSON.validate` used by both directions, which rejects unknown types, requires each type's content, and reports invalid tool-call arguments by name; the null case returns early. I took the reviewer's `[]byte`-for-args suggestion no further than that — base64-encoding tool arguments would make a persisted history unreadable, and truncated arguments already fail in both request builders, so failing at the boundary with a clear message is the consistent answer.

Smaller review findings, also taken: a stale `[PartMetadata]` doclink left in an anthropic comment, `/AGENTS.md` claiming a lossless round-trip the `ToolResult.Err` caveat contradicts, `eval.Sample` turning an empty-but-non-nil part slice into `null` where it used to be `[]`, and three test gaps — the metadata wire shape was never pinned (a renamed JSON tag would have stranded every stored history while `reflect.DeepEqual` stayed green), anthropic had no wire-level foreign-metadata assertion to match google's, and nothing covered the null/unknown-type/invalid-argument cases. Not taken: exporting `errPartMetadataInvalid` so callers can match it with `errors.Is`, since `errLastMessageEmpty` set the package's precedent of keeping these unexported.

### Why

An interface field is the one Go shape that `encoding/json` can marshal but never unmarshal, and the feature's whole purpose is a history that survives storage. A `{source, bytes}` struct trades compile-time typing — which bought little, since the data was opaque by design — for a format any store can hold. Import paths as the discriminator make collisions impossible and make a persisted envelope self-describing.

Anthropic's encoding is JSON rather than a tagged or length-prefixed form because two fields have to share one byte slice, the package already depends on `encoding/json`, and a persisted envelope a human can read while debugging a broken history is worth more than the few bytes a compact encoding would save.

The `UnmarshalJSON` validation is the same judgment Step 3 made for `errLastMessageEmpty`: a malformed history is caller data, so it fails at the boundary with an error rather than panicking three call frames later.

### What worked

Red-green on the two claims that matter. Deleting `Metadata` from the `partJSON` literal and re-running the new tests: `chat_complete_test.go:38: Expected "example.com/client", but got "" (type string)` in core, `chat_complete_test.go:868: Expected "sig-123", but got "" (type string)` in google's serialized-history test, and `chat_complete_test.go:698: Expected "3", but got "1" (type int)` in anthropic's. Restoring the line made all three pass.

The live flows were green first try and twice in a row: google `-count=2` on the two round-trip subtests plus the foreign-metadata one, anthropic `-count=2` across all five metadata subtests. Anthropic's Sonnet 5 non-determinism showed up again exactly as Step 2 described (`thoughtParts=0` on the first run, `thoughtParts=2 foundSignature=true` on the second), so the conditional assertion earns its keep; the serialized variant saw the signed path on the run where the model chose to think.

### What didn't work

Nothing broke in the implementation. Both full client suites passed on the first run (`./clients/google` 143s with the usual service-account skips, `./clients/anthropic` 79s), as did core, `tools`, `robust`, `eval`, and the examples.

Two red results, neither mine. The whole `./clients/openai` package fails on this machine with `401 Unauthorized ... "Incorrect API key provided: sk-proj-****3S0A", "code": "invalid_api_key"` on every live subtest — the same environmental key problem Steps 2 and 5 recorded, one step worse (the key now reads as invalid rather than under-scoped). The openai client is untouched by this change and every failure is an auth error raised before any part handling.

The other was self-inflicted: the first red run reported anthropic failing at "Expected 3, but got 1" on what I misread as the message count. Logging the request body showed all three messages present and the failure one line lower, on `len(sent.Messages[1].Content)` — the model turn had lost two of its three blocks. The red check was right; my reading of the line number was not.

Also worth recording for whoever runs commands in this worktree: the session refuses any shell command containing the literal string `eval`, so `go test ./eval` has to be written as `go test ./ev*/`.

Both full client suites were re-run after the self-review fixes and stayed green (`./clients/google` 146s, `./clients/anthropic` 71s), as did lint at 0 issues.

### What I learned

`encoding.TextMarshaler` on a struct is a trap for anything that also wants to be data. `Part` looked serializable — exported fields, plain types — and silently was not, in both directions, and no test caught it because nothing in the repo unmarshalled a `Part`. The lesson generalises: if a type is meant to be persisted, a test that marshals *and unmarshals* it is the only proof, and a display-oriented `MarshalText` and a storage-oriented `MarshalJSON` should coexist deliberately rather than by accident.

The second lesson is that the blast radius of adding `MarshalJSON` to a widely-embedded type is not local. `eval.Sample` is two packages away and its output file is a documented artifact; nothing in the type system would have pointed there.

### What was tricky

Deciding how far "make metadata survive serialization" reaches. A metadata-only fix is impossible — `MarshalText` swallows every field — so the honest fix is a lossless `Part` wire format, which drags in the unexported content fields and `ToolResult.Err`. Error identity cannot survive JSON, so the choice was between dropping the error (silent), failing to marshal (hostile), or degrading it to its message (documented). The `evals.jsonl` collision was the other one: preserving that file's shape needed a change in a package the brief never mentioned, and the alternative — letting the report format change — would have silently regressed a deliberate #168 decision.

### What warrants review

The exported surface is two names and one behaviour change: `gai.PartMetadata{Source, Data}`, and `gai.Part` now marshalling to a JSON object instead of a string. The latter is breaking for any caller who JSON-logged messages and expected the summary text; `eval.Sample` is the only in-repo case and is preserved. Then: the `ToolResult.Err`-as-message degradation, `partJSON.validate` (both that marshalling now refuses a malformed part rather than writing it, and that an unknown part type is rejected instead of passed through for forward compatibility — the alternative is the panic it replaces), the choice of import paths as discriminators, anthropic's JSON-in-bytes encoding and its new `errPartMetadataInvalid` path, and whether `eval.Sample.MarshalJSON` belongs in this PR at all.

Validate offline with `go test -count=1 . ./ev*/ ./clients/google ./clients/anthropic -run 'TestPart_MarshalJSON|TestSample_MarshalJSON|TestChatCompleter_ChatComplete/(surfaces_a_signature|sends_a_sig|ignores_metadata|errors_)'`, and live with `go test -count=2 ./clients/google -run 'TestChatCompleter_ChatComplete/(can_round-trip|ignores_foreign)'` and `go test -count=2 ./clients/anthropic -run 'TestChatCompleter_ChatComplete/(can_round-trip|can_resend|ignores_thought)'`.

### Future work

Unchanged from Step 5 — the model-drift conformance triage and the `SA1019` deprecation on `/clients/google/client.go:60` — plus one new item: `gai.Message`, `gai.ToolCall`, and `gai.ToolResult` now have a de-facto persisted wire format through `Part`, and nothing documents or versions it. If histories ever need to migrate across gai versions, that is the thing to pin down first.
