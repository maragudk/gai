...
- Curation policy: export stable, generally-available models of the current and recent generations; previews case-by-case. Dated snapshots and modality variants stay out. A model the provider has killed server-side is removed immediately regardless of recency; superseded-but-alive models age out as the generation window rolls. The policy is stated in an identically-phrased comment above each client's const block. Existing constant names are not renamed; `google` keeps bare strings.
