...
## 2026-08-17: Curate model constants per client and enforce the list with a live conformance test

Each client (`openai`, `anthropic`, `google`) exports a hand-curated set of model constants covering the current plus one-to-two recent generations, enforced in both directions by a live-API conformance test. Triggered by OpenAI deprecating `gpt-5.3-chat-latest` server-side (removed in PR #333), which broke CI on main and surfaced that the constant lists rot silently: a `wantErr` test row still "passes" when a model dies, because a 404 is an error too.

Context: the clients hand-pick model constants, which lag SDK releases (each new model needs a hand-written PR like #325) and outlive server-side deprecations (the SDK retains dead models — openai-go v3.50.0 still ships `ChatModelGPT5_3ChatLatest`). A full SDK mirror was considered and rejected: it would re-import corpses and dated snapshots, hurt GoDoc discoverability, has no mechanical enforcement in Go (constants cannot be enumerated at runtime), and Google's genai SDK exports no model constants to mirror at all.

Decision, three parts:

- Curation policy: export stable, generally-available models of the current and recent generations; previews case-by-case. Dated snapshots and modality variants stay out. A model the provider has killed server-side is removed immediately regardless of recency; superseded-but-alive models age out as the generation window rolls. Alive-but-unusable models get the same treatment as dead ones: a model that cannot work through the client's implemented API surface (e.g. Responses-API-only models such as the `gpt-*-pro` family, which reject the chat-completions endpoint gai's `ChatCompleter` uses) is not exported — it goes on the ignore list with a caveat comment, since a constant whose only possible use is a runtime API error is a trap, not an API. The policy is stated in an identically-phrased comment above each client's const block. Existing constant names are not renamed; `google` keeps bare strings.
- Conformance test: each client package gets a live `TestModelConformance` in the normal CI test job. It resolves every exported chat and embed constant via the provider's get-by-ID endpoint (per-ID lookup, because alias IDs such as `claude-sonnet-4-5` resolve on lookup but do not always appear in list output) and fails on 404. It also fetches the provider's full model list and fails on any ID that is neither exported nor matched by a package-private ignore list supporting exact IDs and prefix patterns (prefixes absorb the steady stream of dated snapshots).
