...
- Multi-turn round-trip of signed thinking blocks (Anthropic's extended-thinking-with-tool-use constraint). The Anthropic client returns a hard error when a `PartTypeThought` part is included in request history (rather than silently dropping it, which previously produced a confusing API 400 several frames later when the unsigned block reached the wire). Multi-turn agents that combine thinking with tool calls must filter thought parts from history until gai grows opaque-metadata support. Tracked at https://github.com/maragudk/gai/issues/250 — solving it requires extending `gai.Part` to carry opaque provider metadata (signature, redacted flag).
