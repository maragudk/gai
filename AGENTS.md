...
- Provider data that must round-trip between turns (Gemini/Anthropic thought signatures) rides on `gai.Part.Metadata` behind the `gai.PartMetadata` marker interface; the `google` and `anthropic` client packages each define their own `PartMetadata` struct, and clients ignore foreign or absent metadata.
