...
- Provider data that must round-trip between turns (Gemini/Anthropic thought signatures) rides on `gai.Part.Metadata`, a `gai.PartMetadata` envelope of a source string plus opaque bytes; the `google` and `anthropic` clients tag it with their own import path, decode only their own, and ignore foreign or absent metadata. `gai.Part` marshals to JSON with its content and metadata intact, so a persisted history can be replayed (a tool-result error travels as its message).
...
- `robust/` wraps `gai.ChatCompleter` and `gai.Embedder[T]` with retry-and-fallback behaviour across multiple underlying implementations.
