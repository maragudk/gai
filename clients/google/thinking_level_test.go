...
func TestChatCompleter_ThinkingLevel(t *testing.T) {
	tests := []struct {
		...
		{name: "panics on a level the client does not publish (xhigh)", level: gai.ThinkingLevel("xhigh"), shouldPanic: true},
		{name: "panics on a level the client does not publish (max)", level: gai.ThinkingLevel("max"), shouldPanic: true},
	}
