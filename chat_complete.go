...
// Not all levels are supported by all providers; unsupported levels panic.
// Anthropic supports only [ThinkingLevelNone] and [ThinkingLevelMinimal] — every
// other level panics there. On both Anthropic and Google, [ThinkingLevelNone]
// is sent as an explicit-disable to the API rather than left unset, so the model
// will not silently fall back to a default thinking mode.
// [ChatCompleteResponseUsage.ThoughtsTokens] semantics differ per provider:
// Google populates it, OpenAI populates it as a token count only (Chat Completions
...
// ThoughtPart creates a thought [Part] holding the model's reasoning content.
// Thought parts are emitted by providers that support extended thinking and
// surface the reasoning trace to the caller separately from the final answer.
...
// ChatCompleteResponseUsage reports per-request token counts.
type ChatCompleteResponseUsage struct {
	// PromptTokens is the number of input tokens charged for the request.
	PromptTokens int
	// ThoughtsTokens is the number of tokens spent on internal reasoning.
	// Cross-provider semantics vary; see [ThinkingLevel].
	ThoughtsTokens int
	// CompletionTokens is the number of output tokens generated by the model.
